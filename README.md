...
- **Clickable product impact pills** — "Affects: Your [Product]" pills in the feed are now clickable buttons. Clicking one opens a Radix UI dialog with a live AI-powered comparison generated by `compareProducts()` via `POST /api/products/compare`.
- **Side-by-side product comparison** — For product launch events, the dialog shows your product vs the competitor's product with images, prices, and descriptions, plus AI-generated strengths, weaknesses, threat level, and strategic recommendation.
- **Impact analysis for other events** — For news, web search, and job posting events, the dialog shows your product details alongside an AI analysis of how the event specifically affects that product.
- **AI competitor similarity scoring** — Manually added competitors now get AI-analyzed similarity scores via `scoreCompetitor()` instead of a hardcoded 70%. Both the setup page and monitoring page show loading animations during analysis, auto-sort by score, and scroll to the newly added competitor.
- **Sidebar redesign** — Navigation links now have icon containers, an active accent pill indicator, and clearer descriptions for each page.
- **Prominent page headers** — All page headers now use a gradient card background (`bg-gradient-to-br from-card to-muted/40`) with rounded borders and shadow, applied globally via the `.page-header` CSS class.
- **Feed sorting improvement** — Feed now sorts with a secondary tiebreaker: date-first mode uses score as tiebreaker (highest score wins within same day), score-first mode uses date as tiebreaker.
- **Resync preserves history** — Removed the `clearModuleEvents` function from the trigger endpoint. Resyncs no longer delete old events and scores — idempotency via `onConflictDoNothing` handles duplicates naturally, preserving historical data.
- **Signal strength type guard** — Fixed a bug where gpt-4o-mini occasionally returned `signal_strength` as a string instead of a number, causing `NaN` in `final_score` and failed DB inserts. Added `typeof` guards in all four modules (news, products, webSearch, jobs).

**New files:**

- `src/components/ui/dialog.tsx` — Radix UI Dialog primitive component
- `src/components/ProductComparisonDialog.tsx` — Product comparison popup with live AI analysis
- `src/app/api/products/compare/route.ts` — AI product comparison endpoint
- `src/app/api/competitors/score/route.ts` — Single-competitor AI similarity scoring endpoint

**New AI functions in `openai.ts`:**

- `scoreCompetitor()` — Evaluates similarity between user's company and a competitor (0-100 + explanation)
- `compareProducts()` — Deep comparison returning headline, strengths/weaknesses, threat level, and recommendation
